...
from ibtcn import TemporalConvNet
...
net.load_state_dict(torch.load('best1.pt'))
...
pred_mean,upper_bound,lower_bound,std = get_confidence_intervals(preds_test,20)
# result = np.array(pred_mean)
y_true = y_seq_test.reshape(-1)
y=np.arange(len(y_true))
#np.savetxt('result19.txt', fmt='%d', delimiter=',')
