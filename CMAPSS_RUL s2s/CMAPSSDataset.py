...
class CMAPSSDataset():
    ...
    def get_nid_sequence(self,input_data,num_id):
        nid_data = input_data[input_data['id']==num_id]
        seqs_num = math.floor(len(nid_data)/self.sequence_length)
        j = len(nid_data) % self.sequence_length
        features = []
        labels = []
        print(len(nid_data))
        print(seqs_num)
        ...
        for i in range(seqs_num):
            feature_seq = nid_data[feature_columns][j:j+self.sequence_length].values
            label_seq = nid_data['RUL'][j:j+self.sequence_length].values
            j = j+self.sequence_length
    ...
    def get_label_slice(self, input_data):
        def reshapeFeatures(input, sequence_length, columns='RUL'):
            data = input[columns].values
            num_elements = data.shape[0]
            for start, stop in zip(range(0, num_elements - sequence_length), range(sequence_length, num_elements)):
                yield (data[start:stop])

        feature_list = [list(reshapeFeatures(input_data[input_data['id'] == i], self.sequence_length))
                        for i in range(1, self.engine_size + 1) if
                        len(input_data[input_data['id'] == i]) > self.sequence_length]
        feature_array = np.concatenate(list(feature_list), axis=0).astype(np.float32)
    ...
    def get_last_data_slice(self, input_data):
        ...
        test_label_list = [input_data[input_data['id'] == i]['RUL'].values[-self.sequence_length:]
                           for i in range(1, num_engine + 1) if
                           len(input_data[input_data['id'] == i]) >= self.sequence_length]
